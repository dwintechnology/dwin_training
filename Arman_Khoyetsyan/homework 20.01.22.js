...
let conterner = document.getElementById("conterner")
let post = foo()


let paginetion1 = document.getElementById("pagination")

function paginetion(postArr) {
    let id = 0
    let arrPost = []
    let arr1 = []
    let i = 0
    ...
    postArr.forEach((post) => {
        ...
        if (id === 20) {
            ...
            buttonDiv.innerHTML = ` ${paigeNum++} `
            buttonDiv.classList.add("paginetionButton")
            arr1 = []
            id = 0
            paginetion1.append(buttonDiv)

        }

    })
    let paginetionButton = document.getElementsByClassName("paginetionButton")

    for (let pagi of paginetionButton) {
        pagi.addEventListener("click", () => {
            let num = pagi.innerHTML
            conterner.innerHTML = ""
            arrPost[num - 1].forEach((post) => {

                let postDiv = document.createElement("div");
                let postDivTitle = document.createElement("div");
                let postDivBody = document.createElement("div");
                let postDelButton = document.createElement("div")



                postDivTitle.innerHTML = post.title
                postDelButton.innerHTML = "X"
                postDivBody.innerHTML = post.body



                postDelButton.classList.add("postDelButton")
                postDivTitle.classList.add("postDivTitle")
                postDivBody.classList.add("postDivBody")
                postDiv.classList.add("postDiv");


                postDivTitle.append(postDelButton)
                postDiv.append(postDivTitle)
                postDiv.append(postDivBody)


                conterner.append(postDiv)
...
let postDelButton = document.getElementsByClassName("postDelButton")


let undo = document.getElementById("Undo")

for (let i of postDelButton) {
